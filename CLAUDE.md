...
- Video **captions** come from a WEB `/player` call: `captions.playerCaptionsTracklistRenderer.captionTracks[]` (each `baseUrl`, `languageCode`, `name`, `kind == "asr"` for auto-generated). Appending `&fmt=vtt` to `baseUrl` returns WebVTT that ExoPlayer parses directly. `getCaptionTracks` fetches these lazily on CC tap, not on the playback hot path. Verified July 2026.
