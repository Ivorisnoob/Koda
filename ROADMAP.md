...
- i18n groundwork (issue #208): the core UI surfaces now read their strings from `res/values/strings.xml` via `stringResource()`/`pluralStringResource()` — app shell and crash prompt, both navigation bars, music Home (Your Mix, Spotlight, home-style picker), the profile switcher, sleep timer, mini player, song options sheet, add-to-playlist sheet, video options sheet, the Settings hub, and the Library's top-level tabs/FAB/empty states. Android 13+ per-app language selection works through `locales_config.xml`. Fifteen translations ship alongside English: pt-BR, es, fr, de, it, ru, uk, pl, tr, ar, hi, id, ja, ko, zh-CN. **Extraction is deliberately partial** — SearchScreen, the settings detail pages, the player styles, VideoHomeContent/SubscriptionsContent and every ViewModel-owned string (snackbars raised from `HomeViewModel` etc.) are still hardcoded; ViewModel strings need a Context-passing pattern rather than a mechanical swap. Follow-up sweeps should extend the same mapping-script approach file by file.
