...
The release-build half mattered as much as the UI. R8 was already minifying releases, so user traces would have arrived as `a.b.c: Unknown source(1)`; `-keepattributes SourceFile,LineNumberTable` keeps files and lines real, and `build.yml` now uploads each build's `mapping.txt` (named by versionCode when attaching to a release) because renamed symbols are only deobfuscatable against the map of the exact build that produced them. KLog deliberately records every level in debug *and* release - release is exactly where no debugger can attach - and the report's level chips decide what leaves the device instead. One drive-by fix rode along: the release workflow's armeabi-v7a artifact was globbing `*arm64-v8a*.apk`, so it had been shipping the arm64 APK under the wrong name.
