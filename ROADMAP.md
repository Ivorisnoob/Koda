...
~~The third is that the browse tree is two nodes wide and both need the network.~~ **Fixed.** The root now carries four offline-first nodes ahead of the two networked ones - Downloaded, Liked Songs (the device-side like store), Recently Played (listening history, deduplicated) and On This Device (the MediaStore library) - each reading stores that cannot come back empty because the network did. Device-local songs arrive carrying their `content://` URI so playback resolves locally rather than dead-ending in stream resolution; history entries for local plays are skipped from Recently Played because history stores ids but not their URIs.

~~The fourth is timing.~~ **Improved.** Recommendations and playlists now serve stale-while-revalidate: a valid cache answers immediately, an expired one answers immediately from cache while exactly one background refresh runs, and only a genuinely cold start waits on the network.
