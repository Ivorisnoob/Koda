...
**Video playback dies a few seconds in, and downloads over ~16 MiB fail.** googlevideo now serves only the first **10-16 MiB of any resolved URL** and answers 403 for everything past it. Re-measured August 2026 against a 335 MiB 1080p stream; this supersedes the earlier "first 1.1 MiB" figure, and the change of scale is what made the old recovery useless rather than merely slow. Three things were ruled out with measurements rather than argument, and each is worth not re-testing: **chunk size is irrelevant** (10 MiB in one request, 16 MiB in eight 2 MiB requests - same wall), **re-resolving cannot resume** (a fresh URL refuses a first request starting at 20, 50, 100 or 200 MiB), and **a PO token on `/player` does not lift it** (identical 403 with and without, on `ANDROID_VR` and `IOS` alike). Bumping the stale `WEB_VERSION` did not help either: four freshly minted tokens under the old pin and four under the current one hit the wall in exactly the same place.

The reason the failure looked like a hang rather than an error is that the first chunk succeeds - the video starts, plays, and only the *second* 10 MiB request is refused, roughly nine seconds in. `refreshVisitorDataAfterPlaybackFailure` then re-mints and re-resolves, the new URL serves its own first chunk, and the loop repeats. The recovery was written when the wall sat at 1.1 MiB, where a 10 MB chunk always overshot and so failed fast and cleanly; at the new distance it papers over the problem instead.

The fix is SABR (`data/SabrSession.kt` and friends, see CLAUDE.md), which reads the whole file. **It is verified on device (August 2026): sustained 1080p playback well past the old wall, the buffer holding ~28s ahead of the playhead and paging continuously.** Getting there took three corrections that are all recorded in CLAUDE.md, and the one worth repeating here is that the last of them was not a protocol bug at all - the server answers with control parts only once its read-ahead window is full, and the loop was reading that as a stall and killing a healthy session 1.5 seconds in.

Two gaps remain. **A forward seek past the download point still fails** rather than merely waiting: player time is clamped to the frontier so the track cannot be corrupted, and the loop then downloads in order towards the seek target, but `SabrDataSource`'s 60s read timeout expires long before a seek of several minutes arrives. A real fix means starting a fresh session and spool at the seek point, which is the next piece of work here. And SABR still covers the video player only - `MusicService`, `ShortsPlayerViewModel` and `DownloadRepository` fetch progressively. Music and Shorts are fine in practice because their content fits under the ceiling; **video downloads are not**.

**The PO token and the visitorData refresh actively fight each other, and the order matters.** A progressive 403 triggers `refreshVisitorDataAfterPlaybackFailure()`, which calls `PoTokenProvider.invalidate()` because the token is bound to the visitorData - so the recovery destroys the very token SABR needs and falls back to progressive again, which 403s again. That loop is why a cold start used to die at 30 seconds no matter how many times it retried. `recoverFromSourceError` now waits up to 6s for a fresh mint after refreshing visitorData. The initial attempt still never blocks on a mint.
